...
function Products() {
  ...
  return (
      ...
      <div className='container mx-auto py-8'>
        <div className='p-5 mb-14'>
          <div className='flex flex-wrap justify-center mb-8 gap-5'>
            {category?.map((btn) => (
              <button
                className={
                  active === btn.id
                    ? 'text-xl px-4 py-3 text-center text-white bg-black border-red border-2  rounded-lg  justify-center font-medium'
                    : 'text-xl px-4 py-3 text-white border-red border-2 font-medium'
                }
                onClick={() => {
                  handleBtn(btn);
                }}
          ...
          <div className="grid py-6  gap-8 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1">
            {
              product?.map((curElem)=>(
                <div className="food-card  bg-white/30 rounded-xl flex flex-col cursor-pointer items-center p-5">
    ...
    <button className='bg-black border-white border-2 active:scale-90 transition duration-150 transform
